...
def _series_supersede_previous_patches(series):
    ...
    for i in range((total_series_revs - 2), (total_series_revs)):
        total_rev_patches = len(Series(series).revisions()[i].ordered_patches())
        # process all patches in current revision
        for l in range(0, total_rev_patches):
            patch_id = Series(series).revisions()[i].ordered_patches()[l].id
            patch_name = Series(series).revisions()[i].ordered_patches()[l].name
            # don't process v# or [##/##] in patch name
            striped_name = re.sub('\s*\[v*\d*\s*\d*/*\d*\s*v*\d*\]\s*', '', 
                                  patch_name)
            # add patch revision-name-id to list
            processed_patches.append(("rev_%s" %(i+1) , striped_name, patch_id,
                                      i, l))
    ...
    for p in range(0, total_processed):
        ...
        if processed_patches[p][0] == 'rev_%s' % total_series_revs:
            for q in range(0, total_processed):
                # Compare names only with patches from previous revision
                if processed_patches[q][0] == 'rev_%s' % (total_series_revs - 1):
                    # if patches have the same name
                    if processed_patches[p][1] == processed_patches[q][1]:
                        # mark patch from previous revision as superseded
                        processed_patch = Patch.objects.get(pk=
                                                        processed_patches[q][2])
                        new_state = State.objects.get(name='Superseded')
                        processed_patch.state = new_state
                        processed_patch.save()
                        # Detail the state change as a comment in superseded patch
                        content = "This is a system generated Comment: Patch %s\
 was automatically marked as superseded by patch %s." \
% (processed_patch.id, processed_patches[p][2])
                        msgid = "System generated by patch %s" \
% processed_patches[p][2]
                        new_comment = Comment.objects.get(patch=processed_patch)
                        try:
                            curr_user = threadlocalrequest.get_current_user()
                            new_comment.submitter = curr_user
                        except ValueError:
                            pass
                        new_comment.content = content
                        new_comment.msgid = msgid
                        new_comment.save()
...
def _on_revision_complete(sender, revision, **kwargs):
    ...
    _series_supersede_previous_patches(series.id)
