...
def _series_supersede_previous_patches(series):
    ...
    for i in range(0, (total_series_revs)):
        ...
        for l in range(0, total_rev_patches):
            ...
            striped_name_pre = re.sub('^(\[(v\d\d*\s*)*(\d\d*/\d\d*\s*)*\
(\(*v\d\d*\)*)*\]\s*)*(^(\(v\d\d*\)\s*)*(v\d\d*\s*))*', '', patch_name,
                                      flags=re.IGNORECASE)
            striped_name = re.sub('(\s*\(v\d\d*\)\s*)*(\s*v\d\d*)*\s*$', '',
                                  striped_name_pre, flags=re.IGNORECASE)
...
%s was automatically marked as superseded by patch %s." \
% (processed_patch.id, processed_patches[p][2])
                        msgid = "%s: System generated by patch %s" \
                            % (datetime.datetime.now().strftime(
                               "%d%b%Y.%H:%M:%S.%f"), processed_patches[p][2])
                        new_comment = Comment.objects.filter(
                            patch=processed_patch)[0]
                        # We should set a "system" user as submitter
                        # for automated comments, otherwise, set as the
                        # latest submitter
                        try:
                            new_comment.submitter = threadlocalrequest.\
                                get_current_user()
                        except ValueError:
                            new_comment.submitter = Patch.objects.get(
                                pk=processed_patches[p][2]).submitter
                        new_comment.pk = None
                        new_comment.headers = msgid
